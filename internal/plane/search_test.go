...
func newSearchRouter(t *testing.T, cfg *plane.PlaneConfig, searcher plane.MemorySearcher) (http.Handler, *bytes.Buffer) {
	...
	return plane.NewServer(cfg, fakeDB{}, &fakeProvisioner{}, nil, searcher, nil, nil, tenant.JWTMiddleware(cfg), logger, nil, nil).Routes(), &logs
}
...
func TestSearchMemoriesFailsClosedWithoutATokenMiddleware(t *testing.T) {
	...
	cfg := newConfig(adminToken)
	router := plane.NewServer(cfg, fakeDB{}, &fakeProvisioner{}, nil, searcher, nil, nil, nil, logger, nil, nil).Routes()
